...
func TestForwardEventBridgePartialFailure(t *testing.T) {
	staticEnv(t)
	stub := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/x-amz-json-1.1")
		fmt.Fprint(w, `{"FailedEntryCount":1,"Entries":[{"ErrorCode":"InternalException","ErrorMessage":"boom"}]}`)
	}))
	defer stub.Close()

	f := New()
	_, err := f.Forward(context.Background(),
		domain.Event{Service: domain.EventServiceEventBridge, Operation: "PutEvents", Target: "default", Source: "s", DetailType: "d", Message: []byte("{}")},
		domain.EventForward{Endpoint: stub.URL, Region: "us-east-1", Credential: "static:test"})
	if err == nil {
		t.Fatal("expected error for failed PutEvents entry")
	}
	if !strings.Contains(err.Error(), "boom") {
		t.Fatalf("error should mention the upstream ErrorMessage, got: %v", err)
	}
}
